...
class BanditResultStore():
    ...
    def _report_junit(self, file_list, scores, excluded_files):
        '''Prints/returns warnings in JUnit (XML) format
        ...
        :return: A collection containing the JUnit (XML) data
        '''
        ...
        root = ET.Element('testsuite', name='bandit', tests=str(len(self.resstore.items())))
        for filename, issues in self.resstore.items():
            testcase = ET.SubElement(root, 'testcase', classname=filename)
            for issue in issues:
                if self._check_severity(issue['issue_severity']):
                    text = 'Severity: %s Confidence: %s \n%s\nLocation %s:%s' % (issue['issue_severity'], issue['issue_confidence'], issue['issue_text'], issue['fname'], issue['lineno'])
                    ET.SubElement(testcase, 'error', type=issue['issue_severity'], message=issue['issue_text']).text = text
    ...
    def report(self, files_list, scores, excluded_files=None, lines=-1,
        ...
        try:
            ...
            if self.format == 'junit':
                self._report_junit(files_list, scores,
                                 excluded_files=excluded_files)
            else:
